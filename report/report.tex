...
A titolo di esempio, il paper \cite{KooBathroom} pubblicato da \textit{Koa et al.} presenta una soluzione di informatizzazione di un bagno privato di un appartamento, grazie all'utilizzo di una serie di sensori suddivisi in "nodi" logici.\\
...
\textit{Koa et al.}, infatti, teorizzano la creazione di un nodo di comunicazione per ogni parte suddivisibile della struttura: uno per il pavimento, uno per il wc, uno per la doccia ecc.
Ognuno di esso verrà connesso a un \textit{Gateway} (un \textit{arduino Yun}) tramite trasmittenti \textit{ZigBee}.
In \textit{Smart Public Restrooms} si può osservare una suddivisione simile: ogni cella singola celle e la struttura nel suo insieme vengono gestite da un singolo \textit{Arduino}, che invia i dati a un \textit{Raspberry Pi} che fa da \textit{Gateway}.\\\\

\begin{figure}[h!]
  \includegraphics[scale=0.45]{img/paperbathroom.png}
  \caption{La soluzione proposta nel paper}
  \label{fig:usecase_manager}
\end{figure}

A livello commerciale, la gestione informatizzata di bagni pubblici è un mercato che si è sviluppato molto rapidamente negli ultimi anni, grazie sopratutto alla maggiore diffusione ed economicità dell'IoT nel mondo.\\
Tuttavia, la maggioranza delle soluzioni disponibili offrono un semplice servizio di reportistica dello stato dei bagni pubblici utilizzati da parte degli utenti; si ritiene possibile affrontare in questo modo le problematiche più stringenti segnalate dagli utenti come mancanza di carta igienica, luci non funzionanti ecc.\\
Al momento sono state individuate le seguenti soluzioni:
\begin{itemize}
\item \textit{txt2clean}\cite{txt2clean} e \textit{restroomalert}\cite{restroomalert}
\item Gli altri due
\end{itemize}2
