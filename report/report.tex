...
\begin{itemize}
	...
	\item Un DBMS per archiviare le informazioni inviate dai clients.
\end{itemize}
...
\subsubsection{DBMS}
\paragraph{Tipologia}
La scelta di un DBMS adeguato deve essere basata sulle reali esigenze dell'applicazione e sul relativo volume di dati.\\
%In precedenza, la sezione di analisi dei requisiti di questo documento aveva riportato una prima approssimativa stima del volume di dati coinvolti durante il funzionamento del sistema; per poter giustificare le scelte progettuali effettuate per l'archiviazione dei dati, viene qui riproposta una stima più accurata.\\\\
%Supponendo un totale di 300 impianti con  	
\paragraph{Sicurezzaè}
...
\paragraph{Configuratore} Per poter efficacemente spiegare il processo di registrazione di un nuovo impianto è necessario introdurre la figura del \textit{configuratore} (o \textit{installatore}) all'interno dell'architettura di sistema.\\
...
Ogni registrazione richiederà l'intervento di due tipologie di utenti: un \textit{configuratore} e un \textit{manager}.\\\\
Come si evince dalla progettazione DBMS, ogni impianto viene identificato da un GUID univoco: esso viene generato casualmente (e univocamente) nella prima fase di registrazione effettuata da un utente \textit{manager}.\\
Attraverso l'applicazione è infatti possibile aggiungere un nuovo \textit{public restroom} compilando alcuni campi obbligatori; al termine di questa operazione verrà consegnato all'utente \textit{manager} il GUID risultante.\\
Tale valore verrà consegnato al \textit{configuratore} che ha predisposto fisicamente l'impianto; una volta ricevuto 
\begin{figure}[h!]
\centering
