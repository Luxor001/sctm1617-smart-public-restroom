...
Applicazioni frontend con un uso intensivo di dati come \textit{Manager} e \textit{Consumer} devono la loro responsività a due fattori principali:
\begin{itemize}
\item \textit{Tempo scaricamento:}
\item \textit{Tempo renderizzazione:}
\end{itemize}
Le applicazioni frontend (\textit{Manager} e \textit{Consumer}) possono risentire 
\subsubsection{Manager}
Come già analizzato nei capitoli precedenti, \textit{Manager} è un applicazione studiata per il monitoraggio di un numero di impianti consistente.
Allo stato attuale del progetto presentato non si prevedono grosse criticitàLe performance riguardanti \textit{Manager}.
\subsubsection{Consumer}
\subsection{Performance parte centrale}
