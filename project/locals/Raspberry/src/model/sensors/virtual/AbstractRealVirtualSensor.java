...
public abstract class AbstractRealVirtualSensor<T extends Object> extends AbstractSensor<T> implements ISensor {
			...
			Optional<Integer> pattern_duration) throws Exception
	{
		...
		if (
				...
				startValue instanceof Vector3)
			lastValue=Optional.ofNullable(this.startValue);
		else if (startValue instanceof Boolean)
			lastValue  = (Optional<T>) Optional.ofNullable(new Integer(0));
		else
	...
	protected void calculatePhase() throws Exception
	{
		...
		case BOOLEAN:
			...
			break;
		case SCALAR:
			phase=Double.parseDouble(this.lastValue.get().toString())/this.maxValue;
		...
		case VECTOR3:
			...
			break;
		default:
			throw new Exception("It's possible to calculate phase only for BOOLEAN, SCALAR or VECTOR3 sensors");
		}
	}
	...
	private void detectClassType()
	{
		if (this.startValue instanceof Boolean)
			this.sensorGeneric=SENSOR_GENERIC.BOOLEAN;
		else if ( this.startValue instanceof Integer || this.startValue instanceof Double)
	...
	protected void updateSinusoidal() {
		...
		else 
		{
			...
			double nextPhase=Math.cos(Double.parseDouble(phase.toString())+pulse*timeLapsed);
			double value=new Double(this.minValue+(range/2)+(range/2)*nextPhase);
			switch(this.sensorGeneric)
			{
			case BOOLEAN:
				if (value>=0.5F)
					this.lastValue=(Optional<T>) Optional.ofNullable(true);
				else
					this.lastValue=(Optional<T>) Optional.ofNullable(false);
				break;
			case SCALAR:
				this.lastValue=(Optional<T>) Optional.ofNullable(value);
				break;
				default: break;
			}
